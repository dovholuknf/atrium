...
The catalog, a review launched at a card, and the lessons view. See `docs/personas-design.md`, stage 6. Run
against a throwaway room. NEVER point it at the real pack: copy it first, and commit the copy so it has a git
history of its own.

### AE0. Set up, once

1. Copy `D:/git/github/dovholuknf/dotagents` to `D:/tmp/pack-copy/dotagents`, keeping its `.git`.
2. In the throwaway room, set `persona_pack_path` to `D:/tmp/pack-copy/dotagents/personas`.
3. Have a card whose worktree is a branch with a commit or two ahead of `main`, with an `origin` remote.

### AE1. The catalog lists the pack

1. Open the runners page, `personas` pane.

**Expected:** one row per persona folder with a `persona.yaml`. Each shows its description, its `reviews` paths
and surfaces, and each declared runner, green when a render exists and marked missing when not. With
`persona_pack_path` empty, the pane says how to set it.

### AE2. Review a card with a persona

1. Right-click the card from AE0 and pick `review with…` then `go-security-reviewer`, and start it.

**Expected:** a new card titled `go-security-reviewer: <card title>`, tagged `origin:agent` and
`persona:go-security-reviewer`, working in `<atrium data>/persona-runs/go-security-reviewer/<run>/`. That folder
holds `TARGET.md` and `.claude/agents/go-security-reviewer.md`. `TARGET.md` names the worktree, the
`<merge-base>..<branch>` range, the repo key from `origin`, and only that repo's knowledge, memory and
rejections. The session answers in the persona's voice, reviews without editing, and its `atrium_report` lands on
the reviewed card. The launched event's `cmd` shows `--agent go-security-reviewer` before the prompt.

### AE3. A persona with no render, or an unmeasured runner, is refused

1. Delete `render/claude/<id>.md` from one persona in the copy. Reopen the card menu.

**Expected:** that persona is not in the `review with…` flyout, and its row marks claude as missing.

### AE4. The lessons view reads git

1. In the copy, commit a change with the message `x` and the trailer line `Lessons-reviewed: codebase-steward`.
2. Edit one of its memory files, add a new untracked one, and append a line to `memory/rejected.md`.
3. Open `lessons` on codebase-steward.

**Expected:** the baseline is the step 1 commit. The edited file shows as modified with its diff, the new one as
untracked, and only the new `rejected.md` line is listed. A persona never reviewed lists its whole memory
folder as unreviewed. `MEMORY.md` is never a row.

### AE5. Promote, keep and delete edit files and commit nothing

1. Promote a lesson with a `repo:` line. Promote one without, naming `general`.
2. Keep a lesson that has no `Why:` line, giving one.
3. Delete a lesson.

**Expected:** promote appends a bullet to `knowledge/<repo key>.md` or `knowledge/_general.md` and removes the
memory file and its `MEMORY.md` line. Keep adds the `repo:` and `**Why:**` lines. Delete removes the file and its
index line. `git status` in the copy shows the changes. `git log` shows no new commit. The view's command block
ends with `-m "Lessons-reviewed: codebase-steward"` and counts what was done.

### AE6. Nothing outside the pack

1. In the copy, replace one persona's `memory/inbox` with a junction to a folder outside the pack that holds a
